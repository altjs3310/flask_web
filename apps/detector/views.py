...
# 매개변수 개수만큼 색상 종류들을 랜덤으로 뽑아내고 그 색상 종류들 중 하나를 선택해서 리턴시켜주는 함수
...
def draw_texts(result_image, line, c1, cv2, color, labels, label):
  ...
  display_text = labels[label] # 숫자인 결과를 매칭되는 물체명으로 바꿔줌
  font = max(line - 1, 1) # 0보다 작을 시 최소값 1
  t_size = cv2.getTextSize(display_text, 0, fontScale=line/3, thickness=font)[0]
  c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3 # c1의 x축 + t_size의 너비, c1의 y축 - t_size의 높이
...
def exec_detect(target_image_path):
  ...
  if tags: # 영역과 이름이 추가된 이미지가 생성(복사본)후 DB에 저장
...
def detect(image_id):
  ...
  user_image.is_detected = True # 감지된 데이터를 True로 바꿔줌
...
def delete_image(image_id):
  try:
    ...
    print(e) # current_app.logger.error(e) = 로그 확인
    db.session.rollback()
...
def search():
  ...
  user_images = db.session.query(User, UserImage)\
                          .join(UserImage).filter(User.id == UserImage.user_id).all()
  ...
  for user_image in user_images:
    ...
    if not search_text: # 검색어가 없을 때 (전체 태그 목록 꺼내오기)
      user_image_tags = db.session.query(UserImageTag)\
                                  .filter(UserImageTag.user_image_id == user_image.UserImage.id).all()
    else: # 검색어가 있을 때 (이미지에 대한 태그 목록 중 검색어를 포함하는걸 꺼내옴)
      user_image_tags = db.session.query(UserImageTag)\
                                  .filter(UserImageTag.user_image_id == user_image.UserImage.id)\
                                  .filter(UserImageTag.tag_name.like(f"%{search_text}%")).all()
      # 해당 검색어를 포함하는 태그가 없을 때
      if not user_image_tags:
        continue # 해당 이미지는 검색결과에서 제외 

      # 있으면 이미지에 대한 모든 태그목록을 다시 가져와서 추가
      user_image_tags = db.session.query(UserImageTag)\
                                  .filter(UserImageTag.user_image_id == user_image.UserImage.id).all()
